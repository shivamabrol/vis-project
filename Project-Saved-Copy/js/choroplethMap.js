      ...
      vis.counties = vis.g.append("g")
        ...
        vis.counties
                  .on('mousemove', (event,d) => {
                    // console.log(d);
                    // console.log(event);
                      const popDensity = d.properties.value != 0 ? `<strong>${d.properties.value}</strong> pop. density per km<sup>2</sup>` : 'No data available'; 
                      d3.select('#tooltip')
                        .style('display', 'block')
                        .style('left', (event.pageX + vis.config.tooltipPadding) + 'px')   
                        .style('top', (event.pageY + vis.config.tooltipPadding) + 'px')
                        .html(`
                          <div class="tooltip-title">State : ${d.properties.state} County: ${d.properties.county}</div>
                          <div>${popDensity}</div>
                        `);
                    })
                    .on('mouseleave', () => {
